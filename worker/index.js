...
async function testConfig(env, corsHeaders) {
  try {
    const usersList = await env.image_kv.get('users_list')
...
async function getSettings(env, username, corsHeaders) {
  try {
    const userData = await env.image_kv.get(`user:${username}`)
    if (userData) {
...
async function saveSettings(request, env, corsHeaders) {
  try {
    ...
    // 保存用户数据
    await env.image_kv.put(`user:${defaultUsername}`, JSON.stringify(userData))

    // 更新用户列表
    let usersList = []
    const existingList = await env.image_kv.get('users_list')
    if (existingList) {
      usersList = JSON.parse(existingList)
    }
    if (!usersList.includes(defaultUsername)) {
      usersList.push(defaultUsername)
      await env.image_kv.put('users_list', JSON.stringify(usersList))
    }
